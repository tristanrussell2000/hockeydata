...
DROP TABLE IF EXISTS AdjustedFenwick;
CREATE TABLE AdjustedFenwick (
    HomeLeadBinned INTEGER,
    HomeCoeff REAL,
    AwayCoeff REAL
);
WITH HomeFenwick AS 
(SELECT (homeScoreAdj-awayScoreAdj) AS homeLead, COUNT() AS ct 
FROM shot_events
...
INSERT INTO AdjustedFenwick
SELECT HomeLeadBinned, HomeCoeff, AwayCoeff FROM FenwickCoeffs;
